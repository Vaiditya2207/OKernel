...
use crate::server::aether::{upload_handler, list_handlers, download_handler, latest_handler, bundle_download_handler, patch_download_handler, telemetry_handler};
...
pub fn create_app(container_manager: Arc<dyn ContainerManagerTrait>) -> Router {
    Router::new()
        ...
        .route("/api/v1/aether/download/patch", get(patch_download_handler))
        .route("/api/v1/aether/telemetry", post(telemetry_handler))
